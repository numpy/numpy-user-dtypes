                      ...
                      double* offset)
{
  ...
  if (NPY_UNLIKELY(get_conversion_factor) == NULL) {
    ...
    get_conversion_factor =
      PyObject_GetAttrString(mod, "get_conversion_factor");
    Py_DECREF(mod);
  ...
  if (!PyTuple_CheckExact(conv) || PyTuple_GET_SIZE(conv) != 2) {
    PyErr_SetString(PyExc_TypeError, "unit error, conversion was not a tuple.");
    return -1;
  }
                                 ...
                                 npy_intp* view_offset)
{
  ...
  if (given_descrs[1] == NULL) {
    ...
    if (get_conversion_factor(((UnitDTypeObject*)given_descrs[0])->unit,
...
static int
unit_to_unit_get_loop(PyArrayMethod_Context* context,
                      int aligned,
