...
static PyObject* quad_getitem(QuadDTypeObject* descr, char* dataptr) {
  ...
  Py_DECREF(
      val_obj);  // Why decrement this pointer? Shouldn't this be Py_INCREF?
  return val_obj;
}

// For two instances of the same dtype, both have the same precision. Return
// self.
static QuadDTypeObject* common_instance(QuadDTypeObject* self,
                                        QuadDTypeObject* other) {
  return self;
}
...
static PyArray_DTypeMeta* common_dtype(PyArray_DTypeMeta* self,
  ...
  if (other->type_num >= 0 && PyTypeNum_ISNUMBER(other->type_num) &&
    ...
    // float128 is the biggest natively supported float. Return it in all cases
    // where other is a number (and not complex).
    Py_INCREF(self);
    return self;
  }
 ...
 */
static PyObject* quaddtype_new(PyTypeObject* NPY_UNUSED(cls), PyObject* args,
                               PyObject* kwargs) {
