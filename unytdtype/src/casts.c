...
int get_conversion_factor(PyObject *from_unit, PyObject *to_unit,
  ...
  PyObject *conv = PyObject_CallFunctionObjArgs(get_conversion_factor, to_unit, NULL);
...
get_conv_factor_auxdata(PyArray_Descr *from_dt, PyArray_Descr *to_dt)
{
  ...
  if (get_conversion_factor(from_unit, to_unit,
                            &res->factor, &res->offset) < 0) {
...
static int unit_to_unit_strided_no_offset(
    PyArrayMethod_Context *NPY_UNUSED(context), char *const data[],
...
static int unit_to_unit_contiguous_offset(
    PyArrayMethod_Context *NPY_UNUSED(context), char *const data[],
    npy_intp const dimensions[], npy_intp const NPY_UNUSED(strides[]),
    conv_auxdata *auxdata) {
  return 0;
...
static int unit_to_unit_strided_offset(
    PyArrayMethod_Context *NPY_UNUSED(context), char *const data[],
    npy_intp const dimensions[], npy_intp const strides[],
    conv_auxdata *auxdata) {
  return 0;
...
static int unit_to_unit_get_loop(PyArrayMethod_Context *context,
                                         int aligned,
  ...
  *out_transferdata = (NpyAuxData *)conv_auxdata;
  ...
  if (aligned && contig && no_offset) {
    *out_loop =
        (PyArrayMethod_StridedLoop *)&unit_to_unit_contiguous_no_offset;
    ...
    *out_loop =
        (PyArrayMethod_StridedLoop *)&unit_to_unit_strided_offset;
  } else {
